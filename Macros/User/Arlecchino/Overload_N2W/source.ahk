...
StartTimerResolution()
try {
    Run_Arlecchino_Overload_N2W_Strict()
} finally {
    ReleaseAll()
    StopTimerResolution()
}
...
Run_Arlecchino_Overload_N2W_Strict() {
    local cycleStart, now

    cycleStart := QpcMs()

    while (ShouldContinue()) {
        if (!WaitUntil(cycleStart))
            return
        if (!ShouldContinue())
            return
        L_Down()

        if (!WaitUntil(cycleStart + 40))
            return
        if (!ShouldContinue())
            return
        L_Up()

        if (!WaitUntil(cycleStart + 80))
            return
        if (!ShouldContinue())
            return
        L_Down()

        if (!WaitUntil(cycleStart + 120))
            return
        if (!ShouldContinue())
            return
        L_Up()

        if (!WaitUntil(cycleStart + 160))
            return
        if (!ShouldContinue())
            return
        L_Down()

        if (!WaitUntil(cycleStart + 200))
            return
        if (!ShouldContinue())
            return
        L_Up()

        if (!WaitUntil(cycleStart + 240))
            return
        if (!ShouldContinue())
            return
        L_Down()

        if (!WaitUntil(cycleStart + 280))
            return
        if (!ShouldContinue())
            return
        L_Up()

        if (!WaitUntil(cycleStart + 320))
            return
        if (!ShouldContinue())
            return
        L_Down()

        if (!WaitUntil(cycleStart + 360))
            return
        if (!ShouldContinue())
            return
        L_Up()

        if (!WaitUntil(cycleStart + 840))
            return
        if (!ShouldContinue())
            return
        W_Down()

        if (!WaitUntil(cycleStart + 880))
            return
        if (!ShouldContinue())
            return
        W_Up()

        if (!WaitUntil(cycleStart + 920))
            return
        if (!ShouldContinue())
            return
        W_Down()

        if (!WaitUntil(cycleStart + 960))
            return
        if (!ShouldContinue())
            return
        W_Up()
